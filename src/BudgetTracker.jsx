...
function BudgetTracker() {
    ...
    const addExpenseRow = () => setExpenses([...expenses, { name: "", value: 0 }]);
            ...
            {income.map((row, index) => (
                <div key={index} className="flex mb-2">
                    ...
                    <input
                        ...
                        className="border p-1 mr-2"
                    />
                    <button onClick={() => removeIncomeRow(index)} className="bg-red-500 text-white p-1">
                        Remove
                    </button>
                </div>
            ))}
            <button onClick={addIncomeRow} className="bg-blue-500 text-white p-2 mb-4">
                Add Income
            </button>
            ...
            {expenses.map((row, index) => (
                <div key={index} className="flex mb-2">
                    ...
                    <input
                        ...
                        className="border p-1 mr-2"
                    />
                    <button onClick={() => removeExpenseRow(index)} className="bg-red-500 text-white p-1">
                        Remove
                    </button>
                </div>
            ))}
            <button onClick={addExpenseRow} className="bg-blue-500 text-white p-2 mb-4">
                Add Expense
            </button>
        </div>
    );
